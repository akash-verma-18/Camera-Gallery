...
capturebtn.addEventListener("click", function () {
    ...
    capturebtn.classList.add("capture-animation"); // to add animation
    // draw a frame on that canvas
    tool.scale(zoomLevel, zoomLevel);
    // to zoom the picture from the centre of the screen and then capture it.
    ...
    // anchor.remove();
    // canvas.remove();
    // i need one second of that animation
    setTimeout(function () {
        capturebtn.classList.remove("capture-animation"); // after 1s it will remove the animation
    }, 1000)
})
function startCounting() {
    ...
    clearObj = setInterval(function () {
        ...
        // Number.parseInt returns an integer
